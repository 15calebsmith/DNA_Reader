...
# TODO: This was just debug stuff. Remove these calls once the ABIF_Reader() class is used by DNA_Reader.py
rdr = ABIF_Reader() # Create an ABIF_Reader object
rdr.read_file("./sample_data/B4 1 Victim Std.fsa", False) # Call read_file for all files using the object you just made
tag_dict = rdr.read_file("./sample_data/C5 PP Ladder-LABEL.fsa", False) # ^
rdr.write_xml("TEST") # This will write the contents of the ABIF_Reader object into an xml
# rdr.print_stats() # Prints stats about the tags
# rdr.print_tags() # Prints a (very long) list of all the tags and their info.
